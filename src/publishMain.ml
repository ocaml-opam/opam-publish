  ...
  let opam_ci_lint =
    ...
    info ["opam-ci-lint"] ~docs ~doc:
      "Run the opam-ci-check-lint tool on the packages before submitting. \
       The lint check runs by default when submitting to ocaml/opam-repository. \
       Any lint errors will prevent the submission, unless --force is used."
...
let main_term root =
  let run
    ...
    let opam_repo_dir =
      PublishSubmit.prepare_opam_repository ~target_branch ~repo root in
    let lint_errors =
